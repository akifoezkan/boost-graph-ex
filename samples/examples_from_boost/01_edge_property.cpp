...
// https://www.boost.org/doc/libs/1_58_0/libs/graph/doc/using_adjacency_list.html#sec:choosing-graph-type 
...
// Creating your own property types and properties - 0 (define property type)
...
void print_network(const Graph& G)
{
  ...
  for (; ui != uiend; ++ui) {
    ...
    for(; out != out_end; ++out)
      cout << "--(" << capacity[*out] << ", " << flow[*out] << ")--> " << target(*out,G) << "\t";

    InEdgeIter in, in_end;
    cout << endl << "\t";
    boost::tie(in, in_end) = in_edges(*ui, G);
    for(; in != in_end; ++in)
      cout << "<--(" << capacity[*in] << "," << flow[*in] << ")-- " << source(*in,G) << "\t";
...
int main(int , char* [])
{
  // Creating your own property types and properties - 2 
  // (bind property type to a value and a graph)
  ...
  // Creating your own property types and properties - 2
  // (read property from a graph)
  property_map<Graph, edge_myflow_t>::type flow = get(edge_myflow, G);

  // Creating your own property types and properties - 3
